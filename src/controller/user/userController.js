...
class UserController {
  ...
  async forgetUpdatePassword(req, res, next) {
    try {
      ...
      return res
        .status(200)
        .json({ message: "User password has been recovered" });
    } catch (error) {
      next(error);
    }
  }
  ...
  async deleteUserById(req, res, next) {
    try {
      ...
      return res
        .status(200)
        .json({ message: `User with ${req.params.id} has been deleted`, user });
    } catch (error) {
      next(error);
    }
  }
    ...
    try {
      ...
      // Check if request body contains 'statusCode'
        ...
        const updatedUserData = await User.findByIdAndUpdate(
          ...
          { $set: { status: req.body.statusCode } }, // Update status with value from request body
